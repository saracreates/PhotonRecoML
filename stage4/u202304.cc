...
void UserEvent202304(PaEvent& e){
    ...
    double del_z; 
    // verwende nur Cluster die nicht direkt am Rand sind
    bool rand = false;
    // verwende nur Cluster, bei denen die Photonen davor nichts machen und ungehindert beim ecal ankommen
    static vector<bool> *no_interaction = new std::vector<bool>(0);
    no_interaction -> clear();
    // speichere keine Zellen doppelt, nur coral nur einen Fit macht
    static vector<int> *index_shower = new std::vector<int>(0);
    index_shower -> clear();
    // speicher keine Events, bei dene Photonen durch das Loch gehen
    bool hole = false;
    ...
    // Primary vertex sollte zwei out going verticies haben
        ...
        if(vertex.IsPrimary()){
            ...
            if(vertex.NMCtrack() == 2){ // zwei outgoing photonen, die sich hoffentlich dann ueberlappen :-) 
                for(int j=0; j<2; ++j){
                    ...
                    // ueberpruefe ob photon durchs Loch geht
                    if(((vertex.Pos(0) + del_x) > 10 and (vertex.Pos(0) + del_x) < 19) and ((vertex.Pos(1) + del_y) > (-4) and (vertex.Pos(1) + del_y) < 4)) hole = true;
                    // speichere Impuls
		    ...
		    momentum -> push_back(photontrack.Pz());
                    ...
                    no_interaction -> push_back((out_vertices.size() != 0) ? false : true); // fancy if else Bedingung Schreibweise (Anmerkung: checke empty -> gibt true and false zurueck)
                }
            } else{
    ...
    // FRAGE: hier schon no_interaction ueberpruefen? Und dann unteren Code gar nicht ausfuehren? JA!!! 
    ...
    for(int i=0; i<e.NCaloClus(); ++i){
        ...
        if(cluster.iCalorim() == 1){ // hier sollte ich zwei mal rein!
	    ...
	    if(cont == true){ // speichere nur einmal Cluster infos, auch wenn Coral falsche Anzahl an Shower fittet
	      for(unsigned int j=0; j<cluster_cellnumbers.size(); ++j){
                ...
                if(icol<=8 || icol>=55 || irow<=12 || irow>=35) rand = true; // grob abscheiden         alternativ return !!!!!!!!!!
                if(((irow>12 && irow<20) || (irow>27 && irow<35)) && (icol<17 || icol>46)) rand = true; // ecken abschneiden
                if(((irow>12 && irow<17) || (irow>30 && irow<35)) && (icol<21 || icol>42)) rand = true; // ecken abschneiden
                if(icol>=33 && icol<=36 && irow>=22 && irow<=25) rand = true; // beamloch rausschneiden
	      }
	    }
        } 
    }
    ...
    if(rand==false and EMC -> size()!=0 and no_interaction -> at(0) == true and no_interaction -> at(1) == true and hole==false) tree->Fill(); // befuelle Tree falls ein schoener Cluster im Event :-) d.h. nicht am Rand und Photon nicht durchs Loch und Photon kommt bei ECAL an und photon nicht durch Loch
